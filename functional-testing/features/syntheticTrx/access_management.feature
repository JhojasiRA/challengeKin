Feature: As an admin of an organization, I want to manage the access to my application, granting, modifying and removing permissions as needed
    ...
    Scenario: Add access to a user to a service with role administrator
        ...
        And the user submits the form with its credentials
        And the user accepts the EULA testing eula "false"
        When the admin adds access to user "testuser21@rockwellautomation.com" to the service "FactoryTalk Vault" with role "Administrator"
        Then the admin should see that the user "testuser21@rockwellautomation.com" has correct access to the resource "FactoryTalk Vault" with role "Admin"
    ...
    Scenario: Add access to a user to a service with role contributor
        ...
        And the user submits the form with its credentials
        And the user accepts the EULA testing eula "false"
        When the admin adds access to user "testuser21@rockwellautomation.com" to the service "FactoryTalk Vault" with role "Contributor"
        Then the admin should see that the user "testuser21@rockwellautomation.com" has correct access to the resource "FactoryTalk Vault" with role "Contributor"

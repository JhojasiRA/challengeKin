...
export var createUserWithToken = async (token: string) => {
  try {
    let url = `${process.env.API_CS}/api/users`;
    let config = {
      headers: {
...
export var getUser = async () => {
  try {
    ...
    var userInfo = {
      ...
      item2: response.data.tenants[0].item2,
    };
    return userInfo;
  } catch (error) {
    return error.response.status;
...
export var getEffectiveRoles = async () => {
  try {
    ...
    while (flag == false) {
      var userInfo = {
        status: response.status,
        resourceType: response.data[count].resourceType,
        role: response.data[count].role,
        resourceName: response.data[count].resourceName,
        resourceType1: response.data[count + 1].resourceType,
        role1: response.data[count + 1].role,
        resourceName1: response.data[count + 1].resourceName,
      };
      if (
        userInfo.resourceType == "Tenant" &&
        userInfo.role == "Owner" &&
        userInfo.resourceName == "Personal Tenant"
      ) {
        tenant = 1;
      }
      if (
        userInfo.resourceType1 == "Service" &&
        userInfo.role1 == "Owner" &&
        userInfo.resourceName1 == "Vault"
      ) {
        service = 1;
      }
